...
const LINE_ID = 'jubilee';
const LINE_DEPTH_M = 30; // heuristic fallback (Jubilee is deep)
...
// load anchors (may be empty for Jubilee)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/jubilee', { recursive: true });
await fs.writeFile('public/data/jubilee/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
