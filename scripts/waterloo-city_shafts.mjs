...
const LINE_ID = 'waterloo-city';
const LINE_DEPTH_M = 24; // heuristic fallback (Waterloo & City is deep)
...
// load anchors (may be empty for Waterloo & City)
let anchors = new Map();
try {
...
await fs.mkdir('public/data/waterloo-city', { recursive: true });
await fs.writeFile('public/data/waterloo-city/shafts.json', JSON.stringify({
  line: LINE_ID,
  origin: ORIGIN,
  verticalScale,
  groundY,
